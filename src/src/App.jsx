...
// This prevents the infinite "Loading your app..." screen if config is missing or malformed
let fConfig = { apiKey: "dummy-key", projectId: "dummy-project", appId: "123" };
try {
  if (typeof __firebase_config !== 'undefined') {
    // Check if it's already an object, or parse it if it's a string
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        setAuthLoading(false); // Stop loading if auth strictly fails
      }
    };
    initAuth();
  ...
  // Fetch Tenant Data (Runs only when in 'tenant' view and authenticated)
  useEffect(() => {
