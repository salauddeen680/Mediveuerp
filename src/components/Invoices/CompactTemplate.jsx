...
const CompactTemplate = ({
  ...
  const inputStyle = "w-full outline-none bg-transparent text-black border-b border-transparent hover:border-gray-400 hover:border-dashed focus:border-teal-600 focus:border-solid font-semibold py-1 transition-colors";
    ...
    <div>
      ...
      <div id="compact-invoice-wrapper" className="w-full overflow-x-auto pb-10 print:pb-0 scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent">
        ...
        <div id="compact-invoice" ref={invoiceRef} className="w-full min-w-[800px] max-w-[210mm] mx-auto bg-white p-6 border border-gray-400 shadow-xl text-black text-sm relative" style={{ minHeight: '1000px' }}>
          ...
          <div className="border-b-2 border-black pb-6 mb-4 relative flex flex-col items-center justify-center text-center">
            ...
            {/* LOGO */}
            {logo && <img src={logo} alt="Store Logo" className="absolute left-0 top-6" style={{ height: '65px', objectFit: 'contain' }} />}
            ...
            <div className="w-full flex flex-col items-center justify-center">
              ...
              {storeSettings.gstin && <p className="text-xs font-bold text-black mt-2">GSTIN: {storeSettings.gstin}</p>}
            </div>
          </div>
          ...
          <div className="flex justify-between border-b border-gray-400 pb-3 mb-4 text-[13px]">
            {/* BILLED TO */}
            <div className="w-1/2 pr-4 flex flex-col gap-2 border-r border-gray-400">
              <div className="font-bold underline mb-1">Billed To (Customer):</div>
              <div className="flex items-center"><span className="w-20 font-bold">Name:</span> <input type="text" className={`${inputStyle} uppercase font-black text-[14px]`} placeholder="Customer Name" value={buyerDetails.name} onChange={e => setBuyerDetails({...buyerDetails, name: e.target.value})} /></div>
            ...
            <table className="w-full text-left border-collapse border border-gray-400">
              <thead>
                <tr className="bg-gray-100 text-[11px] font-extrabold uppercase border-b border-gray-400 tracking-tight">
                  ...
                  <th className="border-r border-gray-400 p-2 pl-2">Product Description</th>
                  <th className="border-r border-gray-400 p-2 text-center w-16">HSN</th>
                  <th className="border-r border-gray-400 p-2 text-center w-12">Qty</th>
                  <th className="border-r border-gray-400 p-2 text-right w-16">Rate</th>
                  <th className="border-r border-gray-400 p-2 text-center w-12">Dis%</th>
                  <th className="border-r border-gray-400 p-2 text-center w-12">GST%</th>
                  <th className="p-2 text-right w-20 pr-2">Amount</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item, index) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-gray-300 text-[13px] group relative align-top hover:bg-gray-50">
                      <td className="border-r border-gray-400 p-2 text-center font-bold relative">
                        {index + 1}
                        {items.length > 1 && (
                      ...
                      <td className="border-r border-gray-400 p-2 pl-2"><input type="text" className={`${inputStyle} font-bold`} placeholder="Type product name..." value={item.description} onChange={(e) => handleItemChange(index, 'description', e.target.value)} /></td>
                      <td className="border-r border-gray-400 p-2"><input type="text" className={`${inputStyle} text-center`} value={item.hsn || ''} onChange={(e) => handleItemChange(index, 'hsn', e.target.value)} /></td>
                      <td className="border-r border-gray-400 p-2"><input type="number" className={`${inputStyle} text-center font-bold`} placeholder="0" value={item.qty || ''} onChange={(e) => handleItemChange(index, 'qty', e.target.value)} /></td>
                      <td className="border-r border-gray-400 p-2"><input type="number" className={`${inputStyle} text-right`} placeholder="0.00" value={item.rate || ''} onChange={(e) => handleItemChange(index, 'rate', e.target.value)} /></td>
                      <td className="border-r border-gray-400 p-2"><input type="number" className={`${inputStyle} text-center`} placeholder="0" value={item.disPercent || ''} onChange={(e) => handleItemChange(index, 'disPercent', e.target.value)} /></td>
                      <td className="border-r border-gray-400 p-2"><input type="number" className={`${inputStyle} text-center`} placeholder="0" value={item.gstPercent || ''} onChange={(e) => handleItemChange(index, 'gstPercent', e.target.value)} /></td>
                      <td className="p-2 text-right font-bold pr-2">{totalAmount.toFixed(2)}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div className="flex justify-between items-stretch text-[13px] border-t border-gray-400 pt-4 absolute bottom-6 left-6 right-6">
            ...
            <div className="w-7/12 pr-6 flex flex-col justify-between">
              ...
              <div className="mt-4 flex items-center bg-gray-50 p-2 border border-gray-200 rounded">
              ...
              <div className="mt-3">
                ...
                <textarea className="w-full bg-transparent outline-none border border-dashed border-gray-400 focus:border-gray-800 print:border-none text-[11px] resize-none h-10 leading-tight pt-1" value={terms} onChange={e => setTerms(e.target.value)} />
              </div>
            </div>
            ...
            <div className="w-5/12 border border-black rounded flex flex-col">
              ...
              <div className="p-3 pt-10 text-right flex flex-col items-center">
