    ...
    <div className="bg-white text-black p-8 rounded-xl max-w-4xl mx-auto font-mono shadow-2xl border border-gray-300">
      ...
      <div className="flex justify-between items-end pt-2">
        ...
        <div className="flex gap-2 print:hidden">
          <button onClick={handleShareWhatsApp} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-xs font-bold shadow cursor-pointer">
            Share WhatsApp
          </button>
          <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-xs font-bold shadow cursor-pointer">
            Print / Save PDF
          </button>
        </div>
...
function TenantBillingView({ data, showToast, user }) {
  ...
  const storeSettings = settings?.general || { storeName: 'PHARMA WHOLESALE' };

  const addToCart = (med) => {
    setCart([...cart, { ...med, qty: 1, originalPrice: Number(med.mrp) || 100 }]);
    setSearchTerm('');
  };
  ...
  const handleGenerateBill = async () => {
    ...
    const billData = {
      ...
      customerName, customerGstin, items: cart, subtotal, discountPercent, discountAmount, total: finalTotal, storeInfo: storeSettings
    };
    ...
    <div className="flex gap-6 h-[calc(100vh-100px)]">
      <div className="flex-1 flex flex-col gap-4">
        <div className="relative">
          ...
          <input className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-white" placeholder="Search medicine..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          {searchTerm && (
            <div className="absolute top-full left-0 right-0 bg-slate-800 border border-slate-700 mt-1 rounded-lg max-h-48 overflow-y-auto z-50">
              {medicines.filter(m => m.name.toLowerCase().includes(searchTerm.toLowerCase())).map(m => (
                <div key={m.id} onClick={() => addToCart(m)} className="p-2 hover:bg-slate-700 cursor-pointer flex justify-between">
                  <span>{m.name}</span> <span className="text-teal-400">₹{m.mrp}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <Card className="flex-1 overflow-y-auto p-0">
          <table className="w-full text-xs">
            <thead className="bg-slate-900 text-slate-400 uppercase">
              <tr><th className="p-2">S.</th><th className="p-2">Product</th><th className="p-2">Qty</th><th className="p-2">MRP</th><th className="p-2 text-right">Amount</th></tr>
            </thead>
            <tbody>
              {cart.map((item, i) => (
                <tr key={i} className="border-b border-slate-800">
                  ...
                  <td className="p-2 text-right text-teal-400">₹{(item.qty * item.originalPrice).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="w-80 flex flex-col justify-between">
        <Card className="space-y-4">
          ...
          <div className="text-lg font-bold text-white flex justify-between"><span>Total:</span> <span className="text-teal-400">₹{finalTotal.toFixed(2)}</span></div>
          <Button onClick={handleGenerateBill} className="w-full">Generate Bill</Button>
        </Card>
      </div>
      {activeInvoice && <Modal isOpen={!!activeInvoice} onClose={() => setActiveInvoice(null)} title="Invoice">{renderMargInvoice(activeInvoice, () => window.print(), () => {})}</Modal>}
    </div>
  );
}
...
function TenantMedicinesView({ data, showToast, user }) {
  ...
  const [form, setForm] = useState({ name: '', batch: 'B1', expiry: '12/26', stock: 50, mrp: 100 });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between"><h1 className="text-2xl font-bold text-white">Inventory</h1><Button onClick={() => setIsModalOpen(true)}>Add Medicine</Button></div>
      <Card className="p-0"><table className="w-full text-sm"><thead className="bg-slate-900 text-slate-400"><tr><th className="p-3">Name</th><th className="p-3">Batch</th><th className="p-3">Stock</th><th className="p-3">MRP</th></tr></thead><tbody>{data.medicines.map(m => <tr key={m.id} className="border-b border-slate-800"><td className="p-3">{m.name}</td><td className="p-3">{m.batch}</td><td className="p-3">{m.stock}</td><td className="p-3">₹{m.mrp}</td></tr>)}</tbody></table></Card>
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Add Medicine"><form onSubmit={handleSave} className="space-y-4"><Input label="Name" value={form.name} onChange={e => setForm({...form, name: e.target.value})} required /><Input label="MRP" type="number" value={form.mrp} onChange={e => setForm({...form, mrp: Number(e.target.value)})} required /><Button type="submit">Save</Button></form></Modal>
    </div>
  );
}
